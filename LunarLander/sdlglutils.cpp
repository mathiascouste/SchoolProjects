...
int XPMFromImage(const char * imagefile, const char * XPMfile)
{
    ...
    for (y = 0; y < (unsigned int)image32bits->h; y ++)
    {
        fprintf(xpm,"\t\t\"");
        for (x = 0; x < (unsigned int)image32bits->w ; x ++)
        {
...
SDL_Surface *load_image( std::string filename ) {
    ...
    if( loadedImage != NULL ) {
        std::cout << "sdlglutils.cpp : C'est ok " << std::endl;
...
GLuint loadText(const char * _text, bool useMipMap)
{
    ...
    police = TTF_OpenFont("/home/user/Documents/projets/cpp/openGL/lunarlander/polices/arial.ttf", 10);

    SDL_Color couleurNoire = {100, 0, 0};

    picture_surface = TTF_RenderText_Solid(police, _text, couleurNoire);
