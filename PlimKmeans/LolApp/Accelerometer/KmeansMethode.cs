...
namespace Accelerometer
{
    class KmeansMethode
    {
        ...
        public static double[][] lastMeans;

            public static void DoKmean(int k, double eps)
        {
            List<DataEntry> data = DataEntry.filtredDatas;
            int cols = DataEntry.getFiltedDataCols();
            int rows = DataEntry.getFiltedDataRows();
            // Console.WriteLine("\nBegin k-means clustering demo\n");
            double[][] rawData = new double[rows][];
            for (int i=0; i<rows; i++)
            {
                rawData[i] = data.ElementAt<DataEntry>(i).getRawDatas();
            }
